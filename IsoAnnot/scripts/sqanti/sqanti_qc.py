...
def correctionPlusORFpred(args, chrom_names):
	...
	print("perl", utilitiesPath+"gmst/gmst.pl", corrFASTA , '-faa', '--strand','direct', '--fnn', '--output',  os.path.splitext(os.path.basename(corrFASTA))[0])
        subprocess.call(["perl", utilitiesPath+"gmst/gmst.pl", corrFASTA , '-faa', '--strand','direct', '--fnn', '--output',  os.path.splitext(os.path.basename(corrFASTA))[0]], cwd=gmst_dir)
        os.rename(gmst_dir+os.path.splitext(os.path.basename(corrFASTA))[0]+".faa", ORF)
	#print("perl", utilitiesPath+"gmst/gmst.pl", corrFASTA , '-faa', '--strand','direct', '--fnn', '--output',  os.path.splitext(os.path.basename(args.isoforms))[0]+"_corrected")
...
def reference_parser(args, chrom_names):
	...
	sys.stdout.write(args.annotation)
	referenceFiles = os.path.splitext(args.annotation)[0] +".genePred"
	sys.stdout.write("referenceFiles")
	...
	if not os.path.exists(utilitiesPath+"gtfToGenePred"):
...
def isoformClassification(args, myQueryTranscripts_list, transcripts_chrom_1exon, transcripts_chrom_exons, transcripts_gene, transcripts_gene_chrom, seq, indelsJunc):
	...
	## read coverage files if provided
 	...
 	outputPathPrefix = args.dir+"/sqanti" # +args.output
